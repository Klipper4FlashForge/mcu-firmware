...
This directory reconstructs that source for the **levelBoard**, and the
repository's `tools/` give you what is needed to do the same for the other
three.

Everything here reads the stock images committed under `stock/`, which come
out of the printer's `control` component; nothing here talks to a printer.
...
Fetches the pinned toolchain, builds `klipper/` -- which is already
upstream at the base commit plus the recovery commit -- with
`levelBoard.config`, and gates on the data dictionary.  Use
`./test/verify.sh` instead for the full byte-exactness gate.  It needs `mcu/levelBoard/stock/levelBoard.dict.json`
(see `extract-dict.py` above); point `STOCK_DICT` elsewhere to override.
Builds use one job by default to keep peak memory bounded.  Set
`MCU_BUILD_JOBS=2` only when the machine has room; larger values are rejected.

## How close it gets

It is byte-identical. Upstream Klipper `6d70050` plus the one recovery
commit in `klipper/` produces
`mcu/levelBoard/stock/levelBoard.bin`, all 26,704 bytes of it, MD5
`156366b40ccc51f5768e083fa8ead210`.

| gate | result |
|---|---|
| differing bytes (`cmp -l`) | **0 of 26,704** |
| image size | **26,704 B**, identical, with no synthetic padding |
...
| vector-table slots disagreeing with stock | **0 of 72** |
| literal pools disagreeing with stock | **0** |
| data dictionary | all 5,663 bytes, including every message id |
| compressed identify blob | all 2,281 bytes as stored in flash |

The memory map is stock's: load address `0x08004000` behind a 16 KiB
bootloader, initial SP `0x20004000`.

The result is verified from the committed tree, not from a working copy:
`test/verify.sh` refuses to run while `klipper/` has uncommitted edits,
then builds with `levelBoard.config` and `olddefconfig` and checks the
MD5, the size, `cmp`, every function address and all 54 handlers.  It was
also confirmed the long way round, by lifting the recovery commit onto a
separate clone of upstream at `6d70050` with `git am` and building there.

The target has been stable across releases. The 1.9.4 levelBoard image
differs from the 1.9.7 one in three bytes: one `movw` inside
`get_mcu_version` at `0x08004CAC`, loading 1001 where 1.9.7 loads 7091.

### The reservation: six dead conditionals

The recovered source contains six conditionals whose two arms are
identical. Each emits nothing -- the arms are cross-jumped after reload
and the load and compare are then deleted -- and each exists only to
change how often a value is referenced, which is what decides the
register it gets. Four are in `ff_eddy_check_trigger`, one in
`ff_eddy_update`, one in `DMA_Init`; `gpio_out_reset` and
`ff_eddy_rebaseline` needed none.

One such construct is a believable engineer's slip. Six is not, and
their distribution is lopsided. **So the recovered tree reproduces
stock's bytes without being stock's source.** The likeliest explanation is that real
code not yet recovered supplies those references as a side effect, and
each conditional stands in for part of it. Two candidates are under
investigation: the three write-only globals `ff_eddy_last_dev`
(`0x20000134`), `ff_eddy_dbg_baseline` (`0x20000138`) and
`ff_eddy_offset` (`0x2000019C`), which look like the residue of
telemetry or debug statements; and the `value - 1 > 0xfffe` range-check
idiom, which is not how the constant would be written by hand. The
question is open.
